...
% togli la scacchiera per inquadrare solo l'oggetto centrale
mask = false(size(C));
...
for ptIdx = 1:np
    ...
    m = points(ptIdx, :)';
    tracked = m;
    p = [0;0]; % il tentativo iniziale viene salvato e utilizzato per prevedere lo spostamento successivo
    for n = 2:1:88
        ...
        [DX, DY] = gradient(I2);
        for iter = 1:maxIter
            W2 = interp2(I2, Wx+p(1), Wy+p(2));
            r = W2 - W1;
            r = r(:);
            
            Jx = interp2(DX, Wx+p(1), Wy+p(2));
            Jy = interp2(DY, Wx+p(1), Wy+p(2));
            J = [Jx(:) Jy(:)];
...
legend('Punti originali', 'Traiettoria 1', 'Traiettoria 2', ' Traiettoria 3');